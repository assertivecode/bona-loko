...
4. **Align with Core Values**:
   - Verify compliance with the 7 Core Values ([.agents/rules/platform-core-values.md](../../rules/platform-core-values.md)).
   - Ensure assessment and goal-setting flows honor **Humility** (seeing oneself honestly as one is) before demanding **Integrity** (aligning action with ideals).
5. **Identify Edge Cases & Invariants**:
   - Negative inputs, boundary conditions, rate/frequency limits.
6. **Output**: Produce a concise problem statement and requirements list.
...
- [ ] Aligned with the 7 Core Values (Faith, Gratitude, Humility, Integrity, Respect, Empathy, Freedom).
