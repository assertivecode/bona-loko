...
## Objective
Ensure that whenever a user sends a prompt, the agent systematically identifies and activates the relevant skills from `skills/` before formulating implementation plans or writing code.
...
1. **Lifecycle Phase**:
   - *Problem discovery / Business requirements* -> Consult `skills/product/` (`requirements-analysis`, `feature-design`, `acceptance-criteria`).
   - *Architecture & system structure* -> Consult `skills/architecture/` (`modular-monolith`, `cqrs`, etc.).
   - *Coding & delivery* -> Consult `skills/engineering/` (`implementation`, `skill-builder`, etc.).
   - *Quality assurance* -> Consult `skills/quality/` (testing strategy, unit tests, etc.).
...
1. **Consult the Skill Matrix**: Check [skills/CATALOG.md](../../skills/CATALOG.md) for matching triggers and recommended bundles.
